...
const SUPABASE_URL = "https://gpssjlkubeejrktezzoa.supabase.co";

const PRODUCT_BUCKET = "product%20image";
...
function getImageUrl(value: unknown): string | undefined {
  ...
  /*
   * If the database contains a full Supabase Storage URL,
   * extract only the filename and rebuild it using the
   * correct bucket: "product image".
   */
  if (image.includes("/storage/v1/object/")) {
    ...
    if (lastPart) {
      ...
      return `${SUPABASE_URL}/storage/v1/object/public/${PRODUCT_BUCKET}/${encodeURIComponent(
        filename,
      )}`;
    }
  }
  ...
  /*
   * If the database contains only the filename,
  ...
  /*
   * Keep unrelated external image URLs unchanged.
   */
  return image;
}
